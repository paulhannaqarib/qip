...
import type { Interest, UpdateInterestInput } from "@/lib/interests";
...
export default function EditInterestModal({
    ...
    categories: Category[];
    onClose: () => void;
    onUpdate: (id: string, input: UpdateInterestInput) => void | Promise<void>;
}) {
    ...
    useEffect(() => {
        if (!interest) return;
        setNameEn(interest.nameEn || "");
        setNameAr(interest.nameAr || "");
        setCategoryId(interest.categoryId || "");
        setDescription(interest.description || "");
    }, [interest]);

    const canUpdate = useMemo(() => nameEn.trim().length > 0 && categoryId.trim().length > 0, [
        nameEn,
        categoryId,
    ]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <div className="relative w-[560px] max-w-[92vw] rounded-2xl bg-white p-8 shadow-2xl">
                <div className="flex items-start justify-between">
                    <div>
                        <div className="text-xl font-semibold text-slate-900">Edit Interest</div>
                        <div className="mt-1 text-sm text-slate-500">Update the interest details below.</div>
                    </div>
                ...
                <div className="mt-8 flex justify-end gap-3">
                    ...
                    <button
                        ...
                        className={`h-11 rounded-xl px-6 text-sm font-semibold text-white ${
                            canUpdate ? "bg-indigo-900 hover:bg-indigo-800" : "bg-indigo-200"
                        }`}
                    >
                        Update
                    </button>
